  ...
  return (
    <div className="build-page">
      ...
      <div className="build-list-container">
        {creating ? (
          <div className="build-create-form">
            ...
            <div className="build-create-actions">
              ...
              <button className="build-create-cancel" onClick={() => { setCreating(false); setNewName('') }}>
                Cancel
              </button>
            </div>
          </div>
        ) : (
          <button className="build-create-btn" onClick={() => setCreating(true)}>
        ...
        {isLoading && (
          <div className="build-list-loading">Loading...</div>
        )}
        ...
        <div className="build-list-grid">
          {builds?.map((build: Build) => {
            ...
            const pathColor = build.primaryPathId ? PATH_COLORS[build.primaryPathId] : '#785a28'
                ...
                <div className="build-card-info">
                  ...
                  <div className="build-card-meta">
                    ...
                    {keystoneId && KEYSTONE_ICONS[keystoneId] ? (
                      <div className="build-card-rune-combo" style={{ '--build-accent': pathColor } as React.CSSProperties}>
                        <div className="build-card-keystone">
                          <img src={`${DDRAGON}/${KEYSTONE_ICONS[keystoneId]}`} alt="Keystone" />
                        </div>
                        {build.secondaryPathId && PATH_ICONS[build.secondaryPathId] && (
                          <div className="build-card-secondary">
                            <img src={`${DDRAGON}/${PATH_ICONS[build.secondaryPathId]}`} alt="Secondary" />
                          </div>
                        )}
                      </div>
                    ) : build.primaryPathId && PATH_ICONS[build.primaryPathId] ? (
                      <div className="build-card-rune-combo" style={{ '--build-accent': pathColor } as React.CSSProperties}>
                        <div className="build-card-keystone">
                          <img src={`${DDRAGON}/${PATH_ICONS[build.primaryPathId]}`} alt="Path" />
                        </div>
                        {build.secondaryPathId && PATH_ICONS[build.secondaryPathId] && (
                          <div className="build-card-secondary">
                            <img src={`${DDRAGON}/${PATH_ICONS[build.secondaryPathId]}`} alt="Secondary" />
                      ...
                      <span className="build-card-empty-hint">No runes selected</span>
                    )}
